...
PBRParameters m_Params = PBRParameters(vec3(1), 0, 0, vec3(1), vec3(1), vec3(1), 0);
...
float CalculateHardShadows(int shadowmapIndex, vec3 shadowCoords, float bias)
{
	...
	float z = texture(u_DirectionalShadowMap[0], shadowCoords.xy).r;
...
float CalculateSoftShadows(int shadowmapIndex, vec3 shadowCoords, float bias)
{
	...
	vec2 texelSize = 1.0 / textureSize(u_DirectionalShadowMap[0], 0);
	int sampleCount = 1;
	float shadow = 0.0;
	for(int x = -sampleCount; x <= sampleCount; ++x)
	{
		for(int y = -sampleCount; y <= sampleCount; ++y)
		{
			...
			float z = texture(u_DirectionalShadowMap[0], shadowCoords.xy + vec2(x, y) * texelSize).r;
...
float CalculatePCSS(int shadowmapIndex, vec3 shadowCoords, float bias)
{
	...
	vec2 texelSizeMultiplier = 3.0 / textureSize(u_DirectionalShadowMap[0], 0);
	float shadow = 0.0;
	for (int i = 0; i < PCF_SAMPLES; i++)
	{
		...
		float z = texture(u_DirectionalShadowMap[0], shadowCoords.xy + offset).r;
		shadow += shadowCoords.z - bias > z ? 0.0 : 1.0;
	}
...
void main()
{
	...
	for (int i = 0; i < int(u_NumDirectionalLights); ++i)
	{
    ...
    for (int i = 0; i < int(u_NumPointLights); ++i)
    {
		PointLight light = u_PointLights[i];
        int type = int(round(light.u_AttenFactors.w));
