...
namespace DawnStar
{
		...
		: _width(width), _height(height)
	{
		...
		glTextureStorage2D(_rendererID, 1, _internalFormat, static_cast<int>(_width), static_cast<int>(_height));
	...
	void Texture2D::SetData(void* data, uint32_t size)
	{
		...
		//DS_CORE_ASSERT(size == _width * _height * (_dataFormat == GL_RGBA ? 4 : 3), "Data must be entire texture!");
