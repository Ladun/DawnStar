	...
	namespace Utils
	{
...
#else
			glGenTextures(static_cast<int>(count), outID);
#endif
		}
		...
		static void AttachDepthTexture(uint32_t id, int samples, GLenum format, GLenum attachmentType, uint32_t width, uint32_t height)
		{
			...
			else
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, format, static_cast<int>(width), static_cast<int>(height), 0, format, GL_UNSIGNED_BYTE, nullptr);
