...
#define DS_ENABLE_CPU_PROFILING 1
#if DS_ENABLE_CPU_PROFILING
	#define DS_PROFILE_FRAME(...)						OPTICK_FRAME(__VA_ARGS__)
	#define DS_PROFILE_SCOPE(...)						OPTICK_EVENT(__VA_ARGS__)
	#define DS_PROFILE_TAG(NAME, ...)					OPTICK_TAG(NAME, __VA_ARGS__)
	#define DS_PROFILE_SCOPE_DYNAMIC(NAME)				OPTICK_EVENT_DYNAMIC(NAME)
	#define DS_PROFILE_THREAD(...)						OPTICK_THREAD(__VA_ARGS__)
	#define DS_PROFILE_CATEGORY(NAME, CATEGORY)		OPTICK_CATEGORY(NAME, (static_cast<Optick::Category::Type>(CATEGORY)))
#else
	#define DS_PROFILE_FRAME(...)
	#define DS_PROFILE_SCOPE(...)
	#define DS_PROFILE_TAG(NAME, ...)
	#define DS_PROFILE_SCOPE_DYNAMIC(NAME)
	#define DS_PROFILE_THREAD(...)
	#define DS_PROFILE_CATEGORY(NAME, CATEGORY)
#endif